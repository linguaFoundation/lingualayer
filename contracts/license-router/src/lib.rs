...
#[contract]
pub struct LicenseRouterContract;

#[contractimpl]
impl LicenseRouterContract {
...
mod test {
    ...
    fn test_initialize() {
        let env = Env::default();
        env.mock_all_auths();
        let contract_id = env.register_contract(None, LicenseRouterContract);
        let client = LicenseRouterContractClient::new(&env, &contract_id);
